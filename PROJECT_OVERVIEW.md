...
| **Storage** | JSON / SQLite | Lightweight persistence (JSON) or SQL queries (SQLite) |
| **Database** | SQLite3 | Built-in Python SQL database support |
...
│       ├── storage.py         # JSON data storage helpers (DataStore class)
│       └── database.py        # SQLite storage (SQLiteStore, TaskStore, ExpenseStore)
│
...
### 6. SQLite Database (`src/utils/database.py`)

**Purpose**: SQL-based storage alternative to JSON

**Classes**:
- `SQLiteStore` - Base class with CRUD operations
- `TaskStore` - Specialized task queries
- `ExpenseStore` - Specialized expense queries

**TaskStore Methods**:
- `get_by_status()` / `get_by_priority()` - Filter tasks
- `get_overdue()` - Find overdue tasks
- `get_due_soon(days)` - Tasks due within N days
- `get_completion_stats()` - Get todo/doing/done counts

**ExpenseStore Methods**:
- `get_by_category()` / `get_by_date_range()` - Filter expenses
- `get_total_by_category()` - Spending per category
- `get_monthly_totals()` - Monthly breakdown
- `get_total()` - Filtered totals with SQL aggregation

**Utility Functions**:
- `migrate_json_to_sqlite()` - Migrate existing JSON data to SQLite
