...
- **Dual Storage**: Choose between JSON files or SQLite database for data persistence
...
├── data/                   # Data storage
│   ├── tasks.json          # Task data (JSON storage)
│   ├── expenses.json       # Expense data (JSON storage)
│   ├── reminders.json      # Reminder data (JSON storage)
│   └── productivity.db     # SQLite database (SQL storage)
├── src/
...
│       ├── storage.py     # JSON data storage helpers
...
- **SQLite database** for powerful SQL queries and aggregations
- Pandas DataFrames for efficient analytics
- Choice of storage backend based on use case
...
| Database | SQLite3 | SQL-based data storage |
...
6. **SQLite Queries**: Use the SQLite database for advanced querying capabilities

## 🗄️ SQLite Database Usage

The project includes a SQLite database option for more powerful data operations.

### Migrate JSON to SQLite
```bash
python3 -c "from src.utils.database import migrate_json_to_sqlite; migrate_json_to_sqlite()"
```

### Use SQLite Stores in Python
```python
from src.utils.database import TaskStore, ExpenseStore

# Task operations
task_store = TaskStore()
tasks = task_store.load()                    # Get all tasks
overdue = task_store.get_overdue()           # Get overdue tasks
due_soon = task_store.get_due_soon(7)        # Due in 7 days
stats = task_store.get_completion_stats()    # {todo, doing, done, total}

# Expense operations
expense_store = ExpenseStore()
by_category = expense_store.get_total_by_category()  # Spending per category
monthly = expense_store.get_monthly_totals()         # Monthly breakdown
total = expense_store.get_total(category='Food')     # Filtered totals
```
...
# Query tasks
sqlite3 data/productivity.db "SELECT * FROM tasks"

# Query expenses by category
sqlite3 data/productivity.db "SELECT category, SUM(amount) FROM expenses GROUP BY category"
```
